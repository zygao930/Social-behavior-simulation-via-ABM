...
to update-hypothesis
  ...
  [
    ...
    ifelse random-float 1 < false-positive-rate [
      ask patches [set pcolor green]
      set prob-true (power * prob-true) / ((power * prob-true) + (false-positive-rate * (1 - prob-true)))
      set false-positve-count (false-positve-count + 1)
    ]
    ;;false negative -- PUB BIAS
    [
      ask patches [set pcolor red]
...
to-report prob-false-fact-regarded-true
  let total-count count patches
  report false-positve-count / total-count
end
...
516
261
Probability of False Hypothesis
time
Pr(true)
0.0
10.0
...
<experiments>
  <experiment name="experiment" repetitions="40" runMetricsEveryStep="false">
    ...
    <steppedValueSet variable="initial-prior" first="0.05" step="0.05" last="0.95"/>
