...
to go
  ...
  [
    ...
    ask link-neighbors with [not infected?]
      [
        if (((complex-contagion?) or (count link-neighbors with [infected? = true] > 1)))  ;; infect with probability p
        [
          set infected? true
          set color yellow
          set size infected-size
        ...
        if ((complex-contagion?) and (count link-neighbors with [infected? = true] = 1) and (random-float 2 <= prob-spread-one))
        [
...
to-report all-agents
  report count turtles
end

to-report time-to-infect-all
  let start-time ticks
  while [any? turtles with [not infected?]]
  [
    go
  ]
  report ticks - start-time
end
...
1014
227
time-rewiring-probablity(log-log)
log(time)
log(rewiring-probablity)
0.0
10.0
0.0
10.0
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot (log 10 time-to-infect-all) / (log 10 rewiring-probability)"
...
infect-two</setup>
  ...
  <experiment name="prob-spread-one" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>time-to-infect-all</metric>
    <metric>pct-infected</metric>
    <enumeratedValueSet variable="rewiring-probability">
      <value value="0.001"/>
      <value value="0.01"/>
